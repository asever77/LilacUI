...
(function (win, doc, undefined) {
  ...
  */
  ...
  var UiAjax = /*#__PURE__*/function () {
    function UiAjax() {
      _classCallCheck(this, UiAjax);
  ...
  */
  ...
  var UiScrollBar = /*#__PURE__*/function () {
    ...
    _createClass(UiScrollBar, [{
      ...
      value: function init(option) {
        ...
        Global.uiParts.wrapTag('<div class="ui-scrollbar-item"><div class="ui-scrollbar-wrap">', el_scrollbar, '</div></div>'); //++make
        ...
        function eventFn(v) {
          ...
          var _iterator = _createForOfIteratorHelper(el_bar),
              _step;
  ...
  Global.uiScrollBar = function () {
    var uiScrollBar = doc.querySelectorAll('.ui-scrollbar');
    ...
    var _loop = function _loop(i, len) {
      var scrollId = uiScrollBar[i].getAttribute('data-scroll-id');
    ...
    for (var i = 0, len = uiScrollBar.length; i < len; i++) {
      _loop(i, len);
    }
  };

  var UiLoading = /*#__PURE__*/function () {
    function UiLoading() {
    ...
    _createClass(UiLoading, [{
      ...
      value: function show(option) {
        var opt = Object.assign({}, this, option);
        var target = Global.uiParts.selectorType(opt.selector);
        var htmlTag = opt.htmlTag; //중복실행 방지
        ...
        this.timerS = setTimeout(function () {
          ...
          for (var _i = 0, _n = target.children.length; _i < _n; _i++) {
            if (target.children[_i].className === 'ui-loading') {
              uiLoading = target.children[_i];
            }

            console.log(target.children[_i].className);
      ...
      value: function hide(option) {
        ...
        target.dataset.ing = ''; //target.dataset.loading = false;
  ...
  var UIdatepicker = /*#__PURE__*/function () {
    ...
    _createClass(UIdatepicker, [{
      ...
      value: function init(target) {
        var $this = this;
        var elCalendar = document.querySelectorAll('.ui-datepicker-btn');

        var _iterator2 = _createForOfIteratorHelper(elCalendar),
            _step2;
      ...
      key: "datepickerReady",
      value: function datepickerReady(target) {
        console.log('datepickerReady'); //this.elTarget = target;

        var el_target = this.elTarget = target;
        var el_datepicker = this.elDatepicker = el_target.closest('.ui-datepicker');
        var el_input = this.elInput = el_datepicker.querySelector('.ui-datepicker-inp');
        var el_wrap = this.elWrap = el_datepicker.querySelector('.ui-datepicker-wrap');
        var option_period = el_datepicker.getAttribute('data-period');
        var option_dual = el_datepicker.getAttribute('data-dual');
        var option_title = el_datepicker.getAttribute('data-title');
        var option_type = el_datepicker.getAttribute('data-type');
        var option_id = el_datepicker.getAttribute('data-id');
        var option_inputid = el_input.getAttribute('id');
        var option_value = el_input.value;
      ...
      key: "displayCalendar",
      value: function displayCalendar(v) {
        console.log('displayCalendar', v);
        this.buildCalendar(v);
        this.reDisplayCalendar(v);
      }
    }, {
      key: "reDisplayCalendar",
      value: function reDisplayCalendar(v) {
        console.log('reDisplayCalendar');
        this.buildCore(v);
      }
    }, {
      key: "buildCalendar",
      value: function buildCalendar(v) {
        console.log('buildCalendar', v);
        var el_dpwrap = this.elWrap.querySelector('.datepicker-wrap');
        var html_wrap = '';
      ...
      key: "buildCore",
      value: function buildCore(v) {
        console.log('buildCore');
        var el_core = this.elWrap.querySelector('.datepicker-core');
        var _val = this.elInput.value;
        var date = this.newDate;

        var _valDate = _val.split(this.dateSplit);

        var generate = v === 'generate' ? true : false;
        var day = !generate ? date.getDate() : _val === '' ? date.getDate() : Number(_valDate[2]);
        var month = !generate ? date.getMonth() : _val === '' ? date.getMonth() : Number(_valDate[1] - 1);
        var year = !generate ? date.getFullYear() : _val === '' ? date.getFullYear() : Number(_valDate[0]);
        var prevMonth = new Date(year, month - 1, 1);
        var thisMonth = new Date(year, month, 1);
        var nextMonth = new Date(year, month + 1, 1);
        var nextMonth2 = new Date(year, month + 2, 1);
        var firstWeekDay = thisMonth.getDay();
        var nextWeekDay = nextMonth.getDay();
        var prevWeekDay = prevMonth.getDay();
        var daysInMonth = Math.floor((nextMonth.getTime() - thisMonth.getTime()) / (1000 * 60 * 60 * 24));
        var daysInMonth_prev = Math.floor((thisMonth.getTime() - prevMonth.getTime()) / (1000 * 60 * 60 * 24));
        var daysInMonth_next = Math.floor((nextMonth2.getTime() - nextMonth.getTime()) / (1000 * 60 * 60 * 24));

        var _minDay = new Array();

        var _maxDay = new Array();

        var next_m = nextMonth.getMonth();
        var week_day = null;
        var empty_before = daysInMonth_prev - firstWeekDay;
        var empty_after = 0;
        month === 2 ? daysInMonth = (_readOnlyError("daysInMonth"), 31) : '';
        next_m = Number(next_m) + 1; //작년으로 가는 경우

        next_m < 1 ? next_m = 12 : ''; //내년으로 넘어가는 경우

        if (next_m > 12) {
          next_m = 1;
          year = (_readOnlyError("year"), year + 1);
        }
        ...
        for (var i = 1; i <= daysInMonth; i++) {
          ...
          if (week_day === 0) {
            daysInMonth - i < 7 ? html_wrap += '</tr>' : html_wrap += '</tr><tr>';
          }
      ...
      key: "textDate",
      value: function textDate(d, m, y, whatday) {
  ...
  */
  ...
  var UiScrolling = /*#__PURE__*/function () {
    function UiScrolling() {
      _classCallCheck(this, UiScrolling);
    ...
    _createClass(UiScrolling, [{
      key: "move",
      value: function move(option) {
        var opt = Object.assign({}, this, option);
        var psValue = opt.value;
        var duration = opt.duration ? opt.duration : 600;
        var callback = opt.callback;
        var ps = opt.ps;
        var adjust = opt.adjust * -1;
        var focus = Global.uiParts.selectorType(opt.focus);
        var targetEle = Global.uiParts.selectorType(opt.selector);
        var scrollEle = document.documentElement || window.scrollingElement;
        var currentY = targetEle.scrollTop;
        var targetY = targetEle.offsetTop - (adjust || 0);
        console.log(currentY, targetY);
        var _start = currentY;
        var _end = 200;
        var unit = (_end - _start) / duration;
  ...
  Global.uiScrolling = new UiScrolling(); // 	if (p === 'top') {
  // 		$target.stop().animate({ 
  // 				scrollTop : psVal 
  // 			}, { 
  // 				duration: s,
  ...
  netive.common = {
    init: function init() {
      ...
      netive.uiAjax.request({
        src: fristHref,
        callback: function callback(v) {
          netive.uiParts.appendHtml(document.querySelector('#baseMain'), v); // netive.uiScrollBar();
          // netive.uiScrollBar();
    ...
    header: function header(v) {
      ...
      var _iterator = _createForOfIteratorHelper(dep2btn),
          _step;
      ...
      function pageChange(event) {
        ...
        var baseMain = document.querySelector('#baseMain');
